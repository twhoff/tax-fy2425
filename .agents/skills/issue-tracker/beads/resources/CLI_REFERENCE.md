...
```bash
# Sync Beads issue data via Dolt
bd dolt pull                   # Pull issue data from remote
bd dolt push                   # Push local issue changes to remote
bd dolt commit                 # Commit pending changes if auto-commit is disabled
bd dolt show                   # Check connection/configuration status
```

> **Note:** Do not use obsolete top-level `bd sync`. `bd backup sync` is only for
> configured backup destinations, not normal issue-data sync.
...
bd dolt push  # Push issue-data changes to remote
```

**ALWAYS run `bd dolt push` at end of agent sessions** to ensure issue-data changes are pushed to remote.
