...
### Holly / Protected-Main Projects

**`bd worktree create` accepts a full path as its first argument** — it is not
limited to a bare name. If your project uses Holly or has PreToolUse hooks that
block mutations on the main branch, worktrees **must** be placed outside the
repo tree. Creating one inside the repo (e.g. `.worktrees/`) will cause hook
blocks the moment the agent tries to run `rm`, write files, or do any mutation.

```bash
# ✅ Outside the repo — required for Holly / protected-main projects
bd worktree create ../<project>-worktrees/<issue-id>

# ⚠️ Inside the repo — fine for simple projects, blocked in Holly repos
bd worktree create .worktrees/<issue-id>
```

If `.holly/config.toml` has a `[worktrees] base_dir` entry, use that path.
The branch name defaults to the last path component of the argument, so
`bd worktree create ../my-project-worktrees/feat-42` creates branch `feat-42`.

## Commands

```bash
# Create worktree — pass a full path, not just a name
bd worktree create ../<project>-worktrees/<issue-id>  # outside repo (Holly)
bd worktree create .worktrees/my-feature              # inside repo (simple)

# List worktrees
bd worktree list

# Show info about the current worktree (run from inside the worktree)
bd worktree info

# Remove worktree cleanly
bd worktree remove ../<project>-worktrees/<issue-id>  # match the path used at create
```
