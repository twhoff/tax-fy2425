...
**Option 3: Manual Dolt sync**
```bash
bd update issue-1 --claim
bd dolt push  # Push issue-data changes to the configured Dolt remote
bd show issue-1
```
...
### Root Cause
The Dolt server no longer requires ordinary Git for local issue tracking, but
some remote-sync workflows still use a Git/Dolt remote.
